namespace System
{
    ...
    public static class DecimalExtensions
    {
        ...
        {
            if (size < 0) throw new ArgumentException("Size must greater or equals than zero.", nameof(size));
            ...
            if (size < Convert.ToDecimal(Math.Pow(1024, 3)))
                return (size / Convert.ToDecimal(Math.Pow(1024, 2))).ToString("F0") + "MB";
            if (size < Convert.ToDecimal(Math.Pow(1024, 4)))
                return (size / Convert.ToDecimal(Math.Pow(1024, 3))).ToString("F0") + "GB";
            if (size < Convert.ToDecimal(Math.Pow(1024, 5)))
                return (size / Convert.ToDecimal(Math.Pow(1024, 4))).ToString("F0") + "TB";
            if (size < Convert.ToDecimal(Math.Pow(1024, 6)))
                return (size / Convert.ToDecimal(Math.Pow(1024, 5))).ToString("F0") + "PB";
